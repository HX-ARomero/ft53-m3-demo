...
import { characterRepository, genreRepository, platformRepository, videogameRepository } from "../repositories/indexRepository";
  ...
  try {
    ...
    const videogameTransaction = await AppDataSource.manager.transaction(
      ...
      async (transactionalEntityManager) => {
        ...
        let genresArray = [];
        if (genres !== undefined) {
          genresArray = await genreRepository.findBy({
            id: In(genres),
          });
          ...
          videogame.genres = genresArray; //* [ {genre1}, ... ]
        }
        ...
        //* Traer el valor máximo del inventario
        ...
        //* result = { id, ..., inventory: 1 }
        videogame.inventory = result.inventory + 1;
...
export const getVideogames = async (req: Request<{}, {title: string}>, res: Response) => {
  try {
    const { title } = req.query;
    if(title) {
      const videogameByName = await videogameRepository.findByTitle(title.toString());
